...
import { Search, Play, Tv, Info, AlertCircle, ChevronRight, Maximize, Volume2, VolumeX, Pause, X, Server, Menu, ChevronLeft } from "lucide-react";
                ...
                <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-[10px] text-white/40 font-mono truncate">{channel.group?.split(';')[0] || 'Unknown'}</span>
                        </div>
...
const ChannelListItem = ({ channel, isSelected, onSelect, onPreview }) => (
    <div className="px-3 py-1.5">
        <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all group ${isSelected
        ...
        >
            <div className={`w-10 h-7 rounded-lg flex-shrink-0 flex items-center justify-center ${isSelected ? 'bg-white/20' : 'bg-black/40'}`}>
                {channel.logo ? (
                    <img src={channel.logo} alt="" className="w-full h-full object-contain p-1" />
                ) : (
                    <Tv className={`w-4 h-4 ${isSelected ? 'text-white' : 'text-white/20'}`} />
                )}
            </div>

            <div className="flex-1 text-left min-w-0">
                <h4 className="text-sm font-medium truncate font-body">{channel.name}</h4>
                <p className={`text-[11px] truncate ${isSelected ? 'text-white/60' : 'text-white/30'}`}>
                    {channel.group.split(';')[0]}
                </p>
            </div>

            <ChevronRight className={`w-4 h-4 flex-shrink-0 transition-transform ${isSelected ? 'text-white' : 'text-white/10 group-hover:text-white/30'}`} />
        </button>
    </div>
);
...
const CategorySidebar = ({ groups, selectedGroup, onSelectGroup, loading, onClose, isOpen }) => (
    <aside className={`fixed lg:relative inset-y-0 left-0 z-40 w-64 shrink-0 border-r border-white/5 bg-black/40 backdrop-blur-xl flex flex-col overflow-hidden transition-transform lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="p-5 border-b border-white/5 flex items-center justify-between">
            <div>
                <h2 className="text-[11px] font-mono text-white/40 tracking-[.4em] uppercase font-black mb-1">Categories</h2>
                ...
                {loading ? (
                    ...
                    groups.map((group) => (
                        <button
                            ...
                            className={`flex items-center gap-3 w-full px-4 py-3 rounded-xl text-[13px] transition-all group ${selectedGroup === group
                                ? "bg-[var(--color-brand)]/20 border border-[var(--color-brand)]/50 text-[var(--color-brand)]"
                                : "text-white/50 hover:text-white hover:bg-white/5"
                                }`}
                        >
                            <span className={`text-lg ${selectedGroup === group ? "" : "opacity-50 group-hover:opacity-100"}`}>
                                {getGroupIcon(group)}
                            </span>
                            <span className="truncate font-medium flex-1 text-left">{group}</span>
                            {selectedGroup === group && (
                ...
                <div className="flex items-center gap-3 md:gap-6 flex-1">
                    ...
                    <div className="flex flex-col min-w-0">
                        <h1 className="text-xl md:text-2xl font-display tracking-wide flex items-center gap-2 md:gap-3">
                            <span className="hidden sm:inline">LIVE</span> <span className="text-[var(--color-brand)]">CH</span><span className="hidden sm:inline text-[var(--color-brand)]">ANNELS</span>
                        </h1>
                        <span className="text-[8px] md:text-[10px] font-mono text-white/30 uppercase tracking-[.3em]">Discovery</span>
                    </div>
                    ...
                    <div className="hidden sm:flex items-center gap-2 px-3 py-1 bg-white/5 rounded-full border border-white/5 ml-auto md:ml-4">
                        ...
                        <span className="text-[10px] md:text-[11px] font-medium text-white/60">{loading ? "..." : `${filteredChannels.length}`}</span>
                    </div>
                </div>
                    ...
                    <div className="flex-1 flex flex-col bg-black/20 border-t border-white/5 overflow-hidden">
                        ...
                        <div className="px-4 py-3 border-b border-white/5 bg-black/40 backdrop-blur-lg">
                            ...
                            <p className="text-[10px] text-white/20 mt-1">
                                {loading ? "Loading..." : `${filteredChannels.length} available`}
                            </p>
                        </div>
                        ...
                        {loading ? (
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center opacity-30">
                                ...
                                <p className="font-display tracking-widest text-xs">NO CHANNELS</p>
                            </div>
                        ) : (
                ...
                <div className="hidden md:flex flex-1 overflow-hidden">
                    ...
                    <div className="md:w-1/3 lg:w-1/4 flex flex-col bg-black/20 border-r border-white/5 overflow-hidden">
                        ...
                        {loading ? (
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center opacity-30">
                                ...
                                <p className="font-display tracking-widest text-xs md:text-sm">NO CHANNELS</p>
                            </div>
                        ) : (
                            <div className="flex-1 overflow-y-auto">
                                <Virtuoso
                    ...
                    <div className="flex-1 shrink-0 bg-gradient-to-br from-black/60 via-black/40 to-black/80 border-l border-white/5 flex flex-col overflow-hidden">
                        ...
                        <div className="px-4 md:px-5 py-3 md:py-4 border-b border-white/5 bg-black/60 backdrop-blur-xl">
                            <h2 className="text-[11px] font-mono text-white/40 uppercase tracking-[.3em] font-black">Preview</h2>
                            <p className="text-[10px] text-white/20 mt-1">Select a channel to preview</p>
                        </div>
                        ...
                        <div className="flex-1 p-3 md:p-5 overflow-hidden flex flex-col">
                            {previewChannel ? (
                                ...
                                <div className="flex-1 flex flex-col items-center justify-center opacity-40">
                                    ...
                                    <p className="text-xs text-white/60 text-center">No preview</p>
                                </div>
                            )}
                        </div>
