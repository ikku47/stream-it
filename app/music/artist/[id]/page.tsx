...
import type { MusicArtistDetail } from "@/lib/music";

export default function ArtistDetailPage({ params }: { params: { id: string } }) {
  const { data: detail, loading, error } = useMusicDetail<MusicArtistDetail>("artist", params.id);
  ...
  return (
    <>
      <Head>
        <title>{detail?.header.title || "Artist"} – JoyFlix</title>
      </Head>
          ...
          <div className="mx-auto max-w-6xl space-y-10">
            <div className="flex flex-col gap-6 rounded-[28px] border border-white/10 bg-white/5 p-6 md:flex-row md:items-center">
              ...
              <div className="flex-1">
                <p className="text-[11px] uppercase tracking-[0.3em] text-[var(--color-brand)]">Artist</p>
                <h1 className="font-display text-4xl text-white md:text-5xl">{detail.header.title}</h1>
                {detail.header.description && (
                  <p className="mt-3 text-sm leading-6 text-[var(--color-text-mute)] line-clamp-4">
                    {detail.header.description}
                  </p>
                )}
              </div>
            ...
            <div>
              {detail.sections.map((section) => (
                <MusicRow key={section.title} title={section.title} kicker="Artist shelf" items={section.items} onPlay={setNowPlaying} />
              ))}
            </div>
          </div>
        )}
