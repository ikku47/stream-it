...
import type { MusicPlaylistDetail } from "@/lib/music";

export default function PlaylistDetailPage({ params }: { params: { id: string } }) {
  const { data: detail, loading, error } = useMusicDetail<MusicPlaylistDetail>("playlist", params.id);
  ...
  return (
    <>
      <Head>
        <title>{detail?.header.title || "Playlist"} – JoyFlix</title>
      </Head>
          ...
          <div className="mx-auto max-w-5xl space-y-10">
            <div className="flex flex-col gap-6 rounded-[28px] border border-white/10 bg-white/5 p-6 md:flex-row md:items-center">
              ...
              <div className="flex-1">
                <p className="text-[11px] uppercase tracking-[0.3em] text-[var(--color-brand)]">Playlist</p>
